...
object LevelSession:
    ...
    state match
      case currentState: ActiveLevel =>
        state = currentState.copy(
          ...
          quizCorrects = currentState.quizCorrects + (if isCorrect then 1 else 0)
        )
      case Idle => ()
  ...
  def elapsedMinutes: Long =
    state match
      case currentState: ActiveLevel =>
        Duration.between(currentState.startTime, LocalDateTime.now()).toMinutes.max(1)
      case Idle => 0

  /** Restituisce le statistiche correnti della sessione di gioco.
   *
   * @return un [[scala.Option]] contenente una pair (Int, Int) dove:
   * - il primo valore è il numero di risposte date ([[ActiveLevel.quizAttempts]])
   * - il secondo valore è il numero di risposte corrette ([[ActiveLevel.quizCorrects]])
   * Restituisce [[scala.None]] se non vi è alcuna sessione attiva (stato [[Idle]]).
   */
  def currentStats: Option[(Int, Int)] =
    state match
      case currentState: ActiveLevel => Some((currentState.quizAttempts, currentState.quizCorrects))
      case Idle           => None
